...
| **Force-UV** device-side option (always require PIN regardless of RP's `userVerification`) | Phase 6+ | Yubikey has this via `ykman fido access enforce-pin`. Right now if an RP sends `userVerification: discouraged`, the browser skips the PIN even when one is set. Force-UV makes the device always assert UV, ignoring RP hints. Useful for users who want defense-in-depth ("always PIN") even when RPs don't ask. Implementation: add an R-mem-backed `force_uv` flag, set via vendor lt-rpc command or `authenticatorConfig` (CTAP2.1); when true, MakeCred/GetAssertion refuse to proceed without pinAuth even if `uv: discouraged`. Out of scope for Phase 5. |

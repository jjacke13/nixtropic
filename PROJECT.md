...
| **Configurable PIN / touch policies** (Yubikey-style runtime tunables) | Phase 8 polish | Phase 6 M2 shipped option A (auto-on after first setPIN). Phase 8 should make this user-configurable via `authenticatorConfig` (CTAP2.1 cmd 0x0D), plus credProtect extension for per-credential UV policy, plus PIN protocol v2 for token permissions (browser caches more aggressively → fewer PIN prompts). See memory `project_configurable_pin_touch_policies.md` for the full 10-item Phase 8 list. |
| **Configurable per-op touch requirement** | Phase 8 polish | User asked 2026-05-11 to make this togglable like Yubikey OpenPGP. CTAP2 spec mandates UP for signing, but a vendor command could allow time-window touch ("one press = N seconds of consent" — non-spec but useful) or per-credential opt-out via credProtect. Phase 7 OpenPGP card brings the same UX question with `ykman openpgp keys set-touch <never\|cached\|always>` per slot. |
| **rp.id text storage** | Phase 8 polish | Currently `enumerateRPs` response emits hex-of-rpIdHash as `rp.id` stand-in because Phase 6 M3 doesn't persist original rp.id text from MakeCred. fido2-token shows the hex instead of e.g. "webauthn.io". Fix: store rp.id at MakeCred time, emit it back. R-mem per-slot has ~130 B unused. |
| **user.name + user.displayName storage** | Phase 8 polish | Currently `fido2-token -L -k <rp>` shows `(null)` for user.name. We store user_handle (id) only. Audit Med-1 from Phase 6 M4: parse_make_cred should also extract user.name + user.displayName and store them. Bonus: makes the credential-list UX usable. |
| **credProtect extension** | Phase 8 polish | Per-credential UV requirement policy. RPs (and users) can mark certain creds "high-security, always require UV" and others "discoverable, UV optional." Lets us have a low-friction default AND high-security opt-in per credential without flipping global Force-UV. Implementation: extend slot R-mem with credProtect byte (already have unused space); honour in MakeCred + GetAssertion paths. |
| **TROPIC01 firmware version pin at boot** | Phase 8 polish | docs/PHASE-6-PLAN.md §4.8 L2 — deferred from M4 due to budget. Refuse to init FIDO surface if chip FW version is below pinned minimum. Defends against silicon downgrade attacks. ~1 KB flash. |
| **CTAP2 error code precision** | Phase 8 polish | Phase 6 M4 audit Med-1: parse_make_cred returns KEY_STORE_FULL when user_handle > 64 B; should return INVALID_LENGTH. Cosmetic; no security issue (defense-in-depth in slots_alloc still rejects). |
| **Iterator arrays uint8_t** | Phase 8 polish | Phase 6 M4 audit Med-2: credmgmt iterator snapshot arrays use int8_t. Works at SLOTS_MAX=32 (well below 127), but should be uint8_t for future-proofing if SLOTS_MAX ever grows. Trivial fix. |

...
let
  ...
  # Phase 7 M1 — USB CCID interface + T=1 ATR + raw APDU loopback.
  # Verifies pcsc-lite enumerates the reader, ATR matches the spec'd
  # minimal T=1 form (3B 80 81 31 30), and APDU echo returns SW=9000.
  # Requires pcscd running on the host (services.pcscd.enable on NixOS).
  ...
  flash-and-validate-phase7-m1 =
    if firmware == null then
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m1-placeholder";
      ...
      writeShellApplication {
        ...
        text = ''
          ...
          echo "═══════════════════════════════════════════════════════════════"
          echo "  Phase 7 M1: flash-and-validate (CCID + ATR + APDU echo)"
          echo "═══════════════════════════════════════════════════════════════"
          ...
          echo ""
          echo "Step 2/2: Phase 7 M1 validation..."
          exec ${../tools/validate-phase7-m1.sh}
        '';
      };

  # Phase 7 M2 — OpenPGP applet SELECT + GET DATA + gpg --card-status.
  # 6 checks; requires pcscd + libccid w/ cafe:4001 + gnupg on host.
  # See docs/PHASE-7-PLAN.md §5 M2 HW checkpoint.
  validate-phase7-m2 = writeShellApplication {
    name = "nixtropic-validate-phase7-m2";
    runtimeInputs = [ usbutils coreutils gnugrep pkgs.opensc pkgs.pcsc-tools pkgs.pcsclite pkgs.gnupg ];
    text = ''
      set -uo pipefail
      exec ${../tools/validate-phase7-m2.sh} "$@"
    '';
  };

  flash-and-validate-phase7-m2 =
    if firmware == null then
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m2-placeholder";
        text = ''echo "Custom firmware not available in this flake."; exit 1'';
      }
    else
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m2";
        ...
        text = ''
          ...
          echo "═══════════════════════════════════════════════════════════════"
          echo "  Phase 7 M2: flash-and-validate (OpenPGP applet + gpg)"
          echo "═══════════════════════════════════════════════════════════════"
          ...
          # Restart pcscd so it re-probes the freshly enumerated reader.
          sudo systemctl restart pcscd 2>/dev/null || true
          sleep 2

          echo ""
          echo "Step 2/2: Phase 7 M2 validation..."
          exec ${../tools/validate-phase7-m2.sh}
        '';
      };

  # Phase 7 M3 — PIN handling (VERIFY/CHANGE/RESET RC) + PUT DATA +
  # TERMINATE/ACTIVATE.  7 checks via opensc-tool + gpg --card-status.
  # See docs/PHASE-7-PLAN.md §5 M3 HW checkpoint.
  validate-phase7-m3 = writeShellApplication {
    name = "nixtropic-validate-phase7-m3";
    runtimeInputs = [ usbutils coreutils gnugrep pkgs.opensc pkgs.pcsc-tools pkgs.pcsclite pkgs.gnupg ];
    text = ''
      set -uo pipefail
      exec ${../tools/validate-phase7-m3.sh} "$@"
    '';
  };

  flash-and-validate-phase7-m3 =
    if firmware == null then
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m3-placeholder";
        text = ''echo "Custom firmware not available in this flake."; exit 1'';
      }
    else
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m3";
        runtimeInputs = [ dfu-util usbutils coreutils gnugrep pkgs.opensc pkgs.pcsc-tools pkgs.pcsclite pkgs.gnupg ];
        text = ''
          set -euo pipefail
          FW_BIN="${firmware}/firmware.bin"
          ...
          echo "═══════════════════════════════════════════════════════════════"
          echo "  Phase 7 M3: flash-and-validate (PIN + PUT DATA + TERMINATE)"
          echo "═══════════════════════════════════════════════════════════════"
          ...
          echo ""
          echo "Step 2/2: Phase 7 M3 validation..."
          exec ${../tools/validate-phase7-m3.sh}
        '';
      };

  # Phase 7 M4 — key generation (Ed25519 on TROPIC01 slot 29) + PSO:CDS.
  # 8 checks via opensc-tool raw APDUs + gpg --card-status.
  # See docs/PHASE-7-PLAN.md §5 M4 HW checkpoint.
  validate-phase7-m4 = writeShellApplication {
    name = "nixtropic-validate-phase7-m4";
    runtimeInputs = [ usbutils coreutils gnugrep pkgs.opensc pkgs.pcsc-tools pkgs.pcsclite pkgs.gnupg ];
    text = ''
      set -uo pipefail
      exec ${../tools/validate-phase7-m4.sh} "$@"
    '';
  };

  flash-and-validate-phase7-m4 =
    if firmware == null then
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m4-placeholder";
        text = ''echo "Custom firmware not available in this flake."; exit 1'';
      }
    else
      writeShellApplication {
        name = "nixtropic-flash-and-validate-phase7-m4";
        runtimeInputs = [ dfu-util usbutils coreutils gnugrep pkgs.opensc pkgs.pcsc-tools pkgs.pcsclite pkgs.gnupg ];
        text = ''
          set -euo pipefail
          FW_BIN="${firmware}/firmware.bin"
          ...
          echo "═══════════════════════════════════════════════════════════════"
          echo "  Phase 7 M4: flash-and-validate (key gen + PSO:CDS)"
          echo "═══════════════════════════════════════════════════════════════"
          ...
          echo ""
          echo "Step 2/2: Phase 7 M4 validation..."
          exec ${../tools/validate-phase7-m4.sh}
        '';
      };
...
in
{
  ...
  validate-phase7-m1 = {
    type = "app";
    program = "${validate-phase7-m1}/bin/nixtropic-validate-phase7-m1";
    meta.description = "Phase 7 M1 — USB CCID interface + ATR + APDU echo (5 checks, requires pcscd)";
  };
  ...
  flash-and-validate-phase7-m1 = {
    ...
    program =
      if firmware == null then
        "${flash-and-validate-phase7-m1}/bin/nixtropic-flash-and-validate-phase7-m1-placeholder"
      else
        "${flash-and-validate-phase7-m1}/bin/nixtropic-flash-and-validate-phase7-m1";
    meta.description = "DFU-flash open firmware + Phase 7 M1 validation";
  };

  validate-phase7-m2 = {
    type = "app";
    program = "${validate-phase7-m2}/bin/nixtropic-validate-phase7-m2";
    meta.description = "Phase 7 M2 — OpenPGP applet SELECT + GET DATA + gpg --card-status (6 checks)";
  };
  ...
  flash-and-validate-phase7-m2 = {
    ...
    program =
      if firmware == null then
        "${flash-and-validate-phase7-m2}/bin/nixtropic-flash-and-validate-phase7-m2-placeholder"
      else
        "${flash-and-validate-phase7-m2}/bin/nixtropic-flash-and-validate-phase7-m2";
    meta.description = "DFU-flash open firmware + Phase 7 M2 validation";
  };

  validate-phase7-m3 = {
    type = "app";
    program = "${validate-phase7-m3}/bin/nixtropic-validate-phase7-m3";
    meta.description = "Phase 7 M3 — PIN handling + PUT DATA + TERMINATE/ACTIVATE (7 checks)";
  };
  ...
  flash-and-validate-phase7-m3 = {
    ...
    program =
      if firmware == null then
        "${flash-and-validate-phase7-m3}/bin/nixtropic-flash-and-validate-phase7-m3-placeholder"
      else
        "${flash-and-validate-phase7-m3}/bin/nixtropic-flash-and-validate-phase7-m3";
    meta.description = "DFU-flash open firmware + Phase 7 M3 validation";
  };

  validate-phase7-m4 = {
    type = "app";
    program = "${validate-phase7-m4}/bin/nixtropic-validate-phase7-m4";
    meta.description = "Phase 7 M4 — key generation + PSO:CDS (sign) (8 checks)";
  };
  ...
  flash-and-validate-phase7-m4 = {
    ...
    program =
      if firmware == null then
        "${flash-and-validate-phase7-m4}/bin/nixtropic-flash-and-validate-phase7-m4-placeholder"
      else
